...
def setup():
    call(["fw_setenv", "bootcmd"          , "run bootcmd_usb; usb stop; run bootcmd_ubi; reset"])
    ...
    call(["fw_setenv", "mtdparts"         , "mtdparts=orion_nand:1M(u-boot),-(data)"])
    call(["fw_setenv", "mtdids"           , "nand0=orion_nand"])
    call(["fw_setenv", "partition"        , "nand0,2"])
    ...
    call(["fw_setenv", "usb_rootdelay"    , "10"])
    ...
    call(["fw_setenv", "set_bootargs_ubi" , "setenv bootargs ubi.mtd=$ubi_part root=ubi0:$ubi_volume rootfstype=ubifs $mtdparts"])
