...
extern void ASSER_TRAJ_AsservissementMouvementRobot(Pose poseRobot, VitessesRobotUnicycle *vitessesConsignes)
{
    ...
    ASSER_TRAJ_LogAsser("xRoueGauche", NBR_ASSER_LOG_VALUE + 1, poseRobot.x+(0.339/2.0)*cos(poseRobot.angle+PI/2));
    ASSER_TRAJ_LogAsser("yRoueGauche", NBR_ASSER_LOG_VALUE + 1, poseRobot.y+(0.339/2.0)*sin(poseRobot.angle+PI/2));
    ASSER_TRAJ_LogAsser("xRoueDroite", NBR_ASSER_LOG_VALUE + 1, poseRobot.x+(0.339/2.0)*cos(poseRobot.angle-PI/2));
    ASSER_TRAJ_LogAsser("yRoueDroite", NBR_ASSER_LOG_VALUE + 1, poseRobot.y+(0.339/2.0)*sin(poseRobot.angle-PI/2));
...
#endif /* PIC32_BUILD */
            //vitesseNormaliseeCourante = ASSER_TRAJ_VitesseNormalisee(&chemin, compteurPeriode, memoVitesseNormaliseeCourante);
            vitesseNormaliseeCourante = ASSER_TRAJ_VitesseNormalisee_V3(&chemin);   /* Incrementation de chemin.profilVitesse.p */
            //memoVitesseNormaliseeCourante = vitesseNormaliseeCourante;

            if (chemin.profilVitesse.p > 0)
            {
                memoSegmentCourant = -1; /* Forcage de la condition de la boucle while pour au moins une execution de la boucle */
                while (segmentCourant != memoSegmentCourant)
                {
                    ...
                    {
                        /* Correction de la vitesse normalisee du segment courant */
                        ...
                        /* Conversion de la vitesse normalisee pour le segment courant en delta de parametre de la courbe */
                        vitesseSegmentCourant = vitesseNormaliseeCourante * chemin.distance;
                        vitesseNormaliseeSegmentCourantCorrigee = (vitesseSegmentCourant / facteurCorrectionT);

                        /* Limitation de la vitesse par la courbure de la trajectoire */
                        /* vitLimiteCourbure = VITESSE_ANGULAIRE_MAX / ((chemin.profilVitesse.diffThetaCourant / TE) + (VITESSE_ANGULAIRE_MAX / chemin.profilVitesse.delta_tmax));  */     /* delta_t max fonction de la courbure de la trajectoire */
                        vitLimiteCourbure = (chemin.profilVitesse.vmax * TE) / (facteurCorrectionT + ((fabs(chemin.profilVitesse.diffThetaCourant) * ECART_ROUE_MOTRICE * facteurVitesseAngulaireMax) / 2.0));
                        if (vitesseNormaliseeSegmentCourantCorrigee > vitLimiteCourbure)
                        {
                            vitesseNormaliseeSegmentCourantCorrigee = vitLimiteCourbure;
                            vitesseNormaliseeCourante = (vitesseNormaliseeSegmentCourantCorrigee * facteurCorrectionT) / chemin.distance;
                            chemin.profilVitesse.p = (unsigned int)(vitesseNormaliseeCourante / chemin.profilVitesse.delta_t);
                    ...
                    else /* if (chemin.mouvement == ROTATION) */
                    {
                        ...
                        vitesseNormaliseeSegmentCourantCorrigee = vitesseNormaliseeCourante;
                    }

                    parametrePositionSegmentTrajectoire = parametrePositionSegmentTrajectoire + vitesseNormaliseeSegmentCourantCorrigee;
                    //chemin.profilVitesse.distNormaliseeRestante = 1.0 - parametrePositionSegmentTrajectoire;
                    chemin.profilVitesse.distNormaliseeRestante = chemin.profilVitesse.distNormaliseeRestante - vitesseNormaliseeCourante;
...
#ifndef PIC32_BUILD
                            memoVitesseNormaliseeCourante = ((vitesseNormaliseeSegmentCourantCorrigee - parametrePositionSegmentTrajectoire) * facteurCorrectionT) / chemin.distance;
#endif /* PIC32_BUILD */
                            parametrePositionSegmentTrajectoire = parametrePositionSegmentTrajectoire * facteurCorrectionT;
                            vitesseNormaliseeCourante = parametrePositionSegmentTrajectoire / chemin.distance;
#ifndef PIC32_BUILD
                            //memoVitesseNormaliseeCourante = memoVitesseNormaliseeCourante + vitesseNormaliseeCourante;
#endif /* PIC32_BUILD */
                            segmentCourant++;
...
//                ASSER_TRAJ_LogAsser("xCCourant", ASSER_TRAJ_TrajectoireBSpline(&chemin.segmentTrajBS[segmentCourant], parametrePositionSegmentTrajectoire).x);
//                ASSER_TRAJ_LogAsser("yCCourant", ASSER_TRAJ_TrajectoireBSpline(&chemin.segmentTrajBS[segmentCourant], parametrePositionSegmentTrajectoire).y);
            }
            else
            {   
...
#ifndef PIC32_BUILD
                facteurCorrectionTordre2  = (((vitLimiteCourbure * facteurCorrectionT) / chemin.distance) * chemin.profilVitesse.vmax) / chemin.profilVitesse.delta_tmax;
                /* debuggage */
                derivPositionTraj = ASSER_TRAJ_DiffCourbeBSpline(&chemin.segmentTrajBS[segmentCourant], parametrePositionSegmentTrajectoire);
                /************/
                //vitesseAngulaire = (chemin.profilVitesse.diffThetaCourant * vitesseNormaliseeSegmentCourantCorrigee) / TE;
#endif /* PIC32_BUILD */
            }
            else /* if (chemin.mouvement == ROTATION) */
            {
        ...
        {
            vitesseNormaliseeCourante = 0.0;
            memoVitesseNormaliseeCourante = 0.0;
            //printf("parametrePositionSegmentTrajectoire final: %1.3f", parametrePositionSegmentTrajectoire);
...
//         }
    ...
    vitesseAngulaire = (chemin.profilVitesse.diffThetaCourant * ((memoVitesseNormaliseeCourante * chemin.distance) / facteurCorrectionT)) / TE;
...
//    ASSER_TRAJ_LogAsser("yPoseReferenceRobot", poseReferenceRobot.y);
    ...
    */
//    ASSER_TRAJ_LogAsser("vitesseNormaliseeCourante", ((memoVitesseNormaliseeCourante * chemin.profilVitesse.vmax) / chemin.profilVitesse.delta_tmax));
    /*
    ...
    */
//    ASSER_TRAJ_LogAsser("parametrePositionSegmentTrajectoire", parametrePositionSegmentTrajectoire);
...
//    ASSER_TRAJ_LogAsser("yDiffTemporellePoseRefRobot", differentielleTemporellePoseReferenceRobot.y);
    ...
    ASSER_TRAJ_LogAsser("vitLongitudinale", vitessesConsignes->longitudinale);
...
/**********************************************************************/
/*! \brief ASSER_TRAJ_CreerProfilVitesse
 *
 *  \note   Determination du profil de vitesse de consigne a appliquer
 *
 *  \param [in]     traj            pointeur de structure definissant la trajectoire
 */
/**********************************************************************/
static float ASSER_TRAJ_CreerProfilVitesse(Trajectoire *traj)
{
}
...
/**********************************************************************/
/*! \brief ASSER_TRAJ_VitesseNormalisee_V4
 *
 *  \note   Determination de la vitesse normalisee de consigne a appliquer (4ieme version: acceleration et decceleration a profil parabolique)
 *          !! En developpement !!
 *
 *  \param [in]     traj                    pointeur de structure definissant la trajectoire
 *
 *  \return            vitNormFinale     nouvelle vitesse normalisee
 */
/**********************************************************************/
static float ASSER_TRAJ_VitesseNormalisee_V4(Trajectoire *traj)
{
    float vitNormTemp = 0.0;
    float distMinDecc;
    ...
    {   
        ...
        else /* traj->profilVitesse.etat est egal a 0, maintient de la vitesse */
        {   
            /* Vitesse constante */
            //vitNormTemp = vitNormInitiale;
            vitNormTemp = ((float)traj->profilVitesse.p) * traj->profilVitesse.delta_t;
...
static unsigned char ASSER_TRAJ_TestFinAsservissement(Trajectoire *traj, float erDist, float memo_erDist, float tolDist, float erAngle, float memo_erAngle, float tolAngle)
{
    ...
    {
        ret = True;
        ASSER_TRAJ_LogAsser("vitesseAnglePtArret", NBR_ASSER_LOG_VALUE + 1, erAngle-memo_erAngle);
        ASSER_TRAJ_LogAsser("CPTvitesseAnglePtArret", NBR_ASSER_LOG_VALUE + 1, ASSER_TRAJ_GetCompteur());
        
        ASSER_TRAJ_LogAsser("finAsser", NBR_ASSER_LOG_VALUE + 1, poseRobot_asserEnd.x);
        ASSER_TRAJ_LogAsser("finAsser", NBR_ASSER_LOG_VALUE + 1, poseRobot_asserEnd.y);
        ASSER_TRAJ_LogAsser("finAsser", NBR_ASSER_LOG_VALUE + 1, poseRobot_asserEnd.angle);
