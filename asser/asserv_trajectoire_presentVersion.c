...
static float                    vitesse_longitudinale_consigne          =   0.0;
static float                    parametrePositionSegmentTrajectoire     =   0.0;    
//static float                    memoVitesseNormaliseeCourante           =   0.0;
static float                    memo_vitesse_longitudinale_consigne     =   0.0;
...
static float                    ASSER_TRAJ_ConsigneVitesseLongitudinale_V4(Trajectoire *traj, float paramTrajectoire);
...
extern void ASSER_TRAJ_AsservissementMouvementRobot(Pose poseRobot, VitessesRobotUnicycle *vitessesConsignes)
{
    ...
    memo_vitesse_longitudinale_consigne = 0.0;
...
/************************************************************/
            ...
            vitesse_longitudinale_consigne = ASSER_TRAJ_ConsigneVitesseLongitudinale_V4(&chemin, parametrePositionSegmentTrajectoire);
            memo_vitesse_longitudinale_consigne = vitesse_longitudinale_consigne;

            if (chemin.profilVitesse.p > 0)
            {
                /* distance de deplacement sur la periode */
                delta_distance = vitesse_longitudinale_consigne * TE;
...
//                ASSER_TRAJ_LogAsser("xCCourant", NBR_ASSER_LOG_VALUE + 1, ASSER_TRAJ_TrajectoireBSpline(&chemin.segmentTrajBS[segmentCourant], parametrePositionSegmentTrajectoire).x);
...
/* Fin de la dermination de la nouvelle position à suivre */
        ...
        else
        {
            ...
            vitesse_longitudinale_consigne = 0.0;
            //memoVitesseNormaliseeCourante = 0.0;
            memo_vitesse_longitudinale_consigne = 0.0;
...
//         }
    ...
    vitesseAngulaire = (chemin.profilVitesse.diffThetaCourant * ((memo_vitesse_longitudinale_consigne) / facteurCorrectionT)) / TE;

    /* Log de valeurs */
    ASSER_TRAJ_LogAsser("xCCourant", NBR_ASSER_LOG_VALUE + 1, ASSER_TRAJ_TrajectoireBSpline(&chemin.segmentTrajBS[segmentCourant], parametrePositionSegmentTrajectoire).x);
    ASSER_TRAJ_LogAsser("xPoseReferenceRobot", NBR_ASSER_LOG_VALUE + 1, poseReferenceRobot.x);
    ASSER_TRAJ_LogAsser("distNormaliseeRestante", NBR_ASSER_LOG_VALUE + 1, chemin.profilVitesse.distNormaliseeRestante);
...
//    ASSER_TRAJ_LogAsser("yPoseReferenceRobot", poseReferenceRobot.y);
    ...
    */
    ASSER_TRAJ_LogAsser("vitesseLongitudinaleConsigne", NBR_ASSER_LOG_VALUE + 1, memo_vitesse_longitudinale_consigne);
    /*
    ...
    */
    ASSER_TRAJ_LogAsser("parametrePositionSegmentTrajectoire", NBR_ASSER_LOG_VALUE + 1, parametrePositionSegmentTrajectoire);
...
*/
    ASSER_TRAJ_LogAsser("vitLongitudinale", NBR_ASSER_LOG_VALUE + 1, vitessesConsignes->longitudinale);
    /*
...
//            ASSER_TRAJ_LogAsser("log_distance", chemin.segmentTrajBS[iSegment].distance);
    ...
    chemin.profilVitesse.p = (unsigned int)((vitesse_longitudinale_consigne / chemin.distance) / chemin.profilVitesse.delta_t);
...
/**********************************************************************/
/*! \brief ASSER_TRAJ_ConsigneVitesseLongitudinale_V4
 *
 *  \note   Determination de la vitesse longitudinale de consigne a appliquer (3ieme version: profil de vitesse trapezoidal)
 *
 *  \param [in]     traj            pointeur de structure definissant la trajectoire
 *
 *  \return         vitNormFinale   vitesse en m/s
 */
/**********************************************************************/
static float ASSER_TRAJ_ConsigneVitesseLongitudinale_V4(Trajectoire *traj, float paramTrajectoire)
{
