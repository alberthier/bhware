...
static unsigned int             ASSER_TRAJ_Acceleration(ParametresProfilVitesse *ppv, float vitesse_in, float flag_TD, float Te, float pas_dist, float alpha, float vmax, float tempsAcc, float D, float *vitesse_suivante);
...
//            ASSER_TRAJ_LogAsser("log_distance", chemin.segmentTrajBS[iSegment].distance);
    ...
    chemin.profilVitesse.acc_a2 = -chemin.profilVitesse.vmax / ((chemin.profilVitesse.decc_alpha - 1.0) * pow(tempsAcc, 2.0));
    chemin.profilVitesse.vitesse_courante = vitesse_profil_consigne;
    ...
    chemin.profilVitesse.acc_D = ASSER_TRAJ_Distance_decceleration(&chemin.profilVitesse, chemin.profilVitesse.vmax, chemin.profilVitesse.acc_alpha);    
    chemin.profilVitesse.decc_D = ASSER_TRAJ_Distance_decceleration(&chemin.profilVitesse, chemin.profilVitesse.vmax, chemin.profilVitesse.decc_alpha);
    ...
    chemin.profilVitesse.pas_echantillon_distance = PAS; // 2cm
...
static void ASSER_TRAJ_GabaritVitesse(Trajectoire *traj, unsigned int flag_init)
{
    ...
    float           paramPosition = 0.0, vitesse_consigne = 0.0, vitesse_consigne_gabarit = 0.0;
    ...
    while (flag_finTraj == 0)
    {
        ...
        if (g_index_tab_gabarit_vitesse < TAILLE_TAB_GABARIT_VITESSE)
        {
            ...
            g_tab_gabarit_vitesse[g_index_tab_gabarit_vitesse] = vitesse_consigne; //_gabarit;
...
static float ASSER_TRAJ_ProfilConsigneVitesse_V5(Trajectoire *traj, float distanceRestante, float vitesse_initiale, unsigned int flag_TD)
{
    ...
    if (traj->profilVitesse.etat != -1)
    {
        ...
        if (traj->profilVitesse.etat == 1)
        {
            ...
            flag_fin_etat = ASSER_TRAJ_Acceleration(&traj->profilVitesse, vitesse_initiale, flag_TD, TE, traj->profilVitesse.pas_echantillon_distance, traj->profilVitesse.acc_alpha, traj->profilVitesse.vmax, tempsAcc, traj->profilVitesse.acc_D, &vitesse_possible);
        ...
        //if (traj->profilVitesse.distNormaliseeRestante < 0.5)
        {
            ...
            traj->profilVitesse.decc_D = ASSER_TRAJ_Distance_decceleration(&traj->profilVitesse, traj->profilVitesse.decc_vmax, chemin.profilVitesse.decc_alpha);

            vitesse_possible = vitesse_initiale;

            g_distance_suivante = traj->profilVitesse.decc_D;
        }
    }
    else /* traj->profilVitesse.etat est egal a -1 */
    {
        /* Decceleration */
        flag_fin_etat = ASSER_TRAJ_Decceleration(&traj->profilVitesse, vitesse_initiale, flag_TD, TE, traj->profilVitesse.pas_echantillon_distance, traj->profilVitesse.decc_alpha, traj->profilVitesse.decc_vmax, traj->profilVitesse.decc_tempsAcc, traj->profilVitesse.decc_D, &vitesse_possible);
        //vitesse_possible = vitesse_initiale;
        if (flag_fin_etat == 1)
        {
