...
static unsigned int             ASSER_TRAJ_ParcoursTrajectoire(Trajectoire *traj, float delta_distance, unsigned int *segmentCourant, float *paramPoseSegTraj);
...
static float                    ASSER_TRAJ_Coefficient_a_prime(ParametresProfilVitesse *ppv, float vmax, float tempsAcc, float alpha);
...
static float                    ASSER_TRAJ_temps_vs_distance(float a, float distance);
static unsigned int             ASSER_TRAJ_Acceleration(ParametresProfilVitesse *ppv, float vitesse_in, float flag_TD, float Te, float pas, float alpha, float vmax, float tempsAcc, float *vitesse_suivante);
static unsigned int             ASSER_TRAJ_Decceleration(ParametresProfilVitesse *ppv, float vitesse_initiale, float flag_TD, float Te, float pas, float alpha, float vmax, float tempsAcc, float *vitesse_suivante);
static float                    ASSER_TRAJ_ProfilConsigneVitesse_V5(Trajectoire *traj, float paramTrajectoire, float vitesse_initiale, unsigned int flag_TD);
...
/************************************************************/
            ...
            vitesse_profil_consigne = ASSER_TRAJ_ProfilConsigneVitesse_V5(&chemin, parametrePositionSegmentTrajectoire, vitesse_profil_consigne, PAS_TEMPS);
...
extern void ASSER_TRAJ_InitialisationTrajectoire(Pose poseRobot, PtTraj *point, unsigned int nbrePts, unsigned int mouvement)
{
    ...
    float           apr;    /* coefficient de base des profils de vitesse paraboliques */
#ifndef PIC32_BUILD
    Pose            poseTest;
    int             intT;
    float           sommeMask;
...
//            ASSER_TRAJ_LogAsser("log_distance", chemin.segmentTrajBS[iSegment].distance);
    ...
    chemin.profilVitesse.acc_alpha = 0.5;
    chemin.profilVitesse.decc_alpha = 0.35;
    chemin.profilVitesse.temps_chgt_phase = chemin.profilVitesse.acc_alpha * tempsAcc;
        /* determination des coefficients a des profils de vitesse d'acceleration et de decceleration */
    //apr = chemin.profilVitesse.vmax / (tempsAcc * (chemin.profilVitesse.temps_chgt_phase - tempsAcc));
    apr = ASSER_TRAJ_Coefficient_a_prime(&chemin.profilVitesse, chemin.profilVitesse.vmax, tempsAcc, chemin.profilVitesse.acc_alpha);
    chemin.profilVitesse.vitesse_chgt_phase = chemin.profilVitesse.acc_alpha * chemin.profilVitesse.vmax;
    chemin.profilVitesse.acc_a1 = apr * (1.0 - (tempsAcc / chemin.profilVitesse.temps_chgt_phase));
    chemin.profilVitesse.acc_a2 = -apr;
    chemin.profilVitesse.vitesse_courante = vitesse_profil_consigne;
    ...
    chemin.profilVitesse.pas_echantillon_distance = 0.02; // 2cm
...
static unsigned int ASSER_TRAJ_ParcoursTrajectoire(Trajectoire *traj, float delta_distance, unsigned int *segmentCourant, float *paramPoseSegTraj)
{
    ...
    float           delta_param_chemin = 0.0;
    unsigned int    flag_trajectoireTerminee = 0;
...
/**********************************************************************/
/*! \brief ASSER_TRAJ_Coefficient_a_prime
 *
 *  \note   Calcul du coefficient a_prime pour determiner les profils de vitesse
 *
 ...
 *
 *  \return         a_prime coefficent des profils de vitesse
 */
/**********************************************************************/
static float ASSER_TRAJ_Coefficient_a_prime(ParametresProfilVitesse *ppv, float vmax, float tempsAcc, float alpha)
{
    return(vmax / ( (1.0 + alpha) * pow(tempsAcc, 2.0)));
}
...
static float ASSER_TRAJ_Distance_decceleration(ParametresProfilVitesse *ppv, float vmax, float alpha)
{
    ...
    apr = ASSER_TRAJ_Coefficient_a_prime(ppv, vmax);
    bpr = -2.0 * apr * tempsAcc;
    cpr = vmax + apr * pow(tempsAcc, 2.0);
    D1 = ((apr * (1.0 - (tempsAcc / ppv->temps_chgt_phase))) / 3.0) * pow(ppv->temps_chgt_phase,3.0);
    D2 = (apr / 3.0) * (pow(tempsAcc, 3.0) - pow(ppv->temps_chgt_phase, 3.0));
    D2 = D2 + (bpr / 2.0) * (pow(tempsAcc, 2.0) - pow(ppv->temps_chgt_phase, 2.0));
    D2 = D2 + cpr * (tempsAcc - ppv->temps_chgt_phase);
    */

    //return(vmax * tempsAcc * (1.0 + ppv->decc_alpha) / 6.0);
    //return(vmax * tempsAcc * ( (-2.0/3.0) * pow(alpha, 2.0) - alpha));
...
/**********************************************************************/
/*! \brief ASSER_TRAJ_temps_vs_distance
 *
 ...
 *  \param [in]     vitesse_initiale    vitesse a partir de laquelle calculer la prochaine valeur de vitesse
 *
 *  \return         vitesseLimite   vitesse en m/s
 */
/**********************************************************************/
...
static unsigned int ASSER_TRAJ_Decceleration(ParametresProfilVitesse *ppv, float vitesse_initiale, float flag_TD, float Te, float pas, float alpha, float vmax, float tempsAcc, float *vitesse_suivante)
{
    float vitesse_initiale_effective = 0.0;
    float vitesse_suivante_effective = 0.0;
    unsigned int flag_fin_decc = 0;

    if (vitesse_initiale > vmax)
    {
        vitesse_initiale_effective = 0.0;
    }
    else
    {
        if (vitesse_initiale < 0.0)
        {
            vitesse_initiale = vmax;
        }
        else
        {
            vitesse_initiale_effective = vmax - vitesse_initiale;
        }
    }

    flag_fin_decc = ASSER_TRAJ_Acceleration(ppv, vitesse_initiale_effective, flag_TD, Te, PAS, alpha, vmax, tempsAcc, &vitesse_suivante_effective);
    *vitesse_suivante = vmax - vitesse_suivante_effective;
...
static void ASSER_TRAJ_GabaritVitesse(Trajectoire *traj, unsigned int flag_init)
{
    ...
    while (flag_finTraj == 0)
    {
        ...
        {
            g_tab_gabarit_vitesse[g_index_tab_gabarit_vitesse] = ASSER_TRAJ_VitesseLimiteEnVirage(traj, diffThetaTrajectoire);
...
static float ASSER_TRAJ_ProfilConsigneVitesse_V5(Trajectoire *traj, float paramTrajectoire, float vitesse_initiale, unsigned int flag_TD)
{
    ...
    float   distance_restante, distance_decceleration;
    ...
    if (traj->profilVitesse.etat != -1)
    {
        ...
        if (traj->profilVitesse.etat == 1)
        {
            ...
            /* Calcul de la vitesse suivante probable */
            flag_fin_etat = ASSER_TRAJ_Acceleration(&traj->profilVitesse, vitesse_initiale, flag_TD, TE, PAS, traj->profilVitesse.acc_alpha, traj->profilVitesse.vmax, tempsAcc, &vitesse_possible);
        ...
        /* Test de declenchement de la decceleration */
        distance_restante = (traj->profilVitesse.distNormaliseeRestante * traj->distance) - vitesse_possible * TE;     
        ...
        if (distance_restante < distance_decceleration)
        //if (traj->profilVitesse.distNormaliseeRestante < 0.5)
        {
            traj->profilVitesse.etat = -1;  /* Passage en decceleration */

            distance_restante = traj->profilVitesse.distNormaliseeRestante * traj->distance;
            /* calcul d'un nouveau vmax et temps de decceleration pour la decceleration */
            /*
            traj->profilVitesse.decc_tempsAcc = (6.0 / (1 + traj->profilVitesse.decc_alpha)) * ((distance_restante) / traj->profilVitesse.vmax) ;
            traj->profilVitesse.decc_tempsAcc = (floor(traj->profilVitesse.decc_tempsAcc / TE) + 1) * TE;
            traj->profilVitesse.decc_vmax = (6.0 / (1 + traj->profilVitesse.decc_alpha)) * (distance_restante / traj->profilVitesse.decc_tempsAcc);
            */
            traj->profilVitesse.decc_tempsAcc = (3.0 / (2.0 - traj->profilVitesse.decc_alpha)) * ((distance_restante) / vitesse_initiale) ;
            ASSER_TRAJ_LogAsser("decc_tempsAcc_float", NBR_ASSER_LOG_VALUE, traj->profilVitesse.decc_tempsAcc);
            traj->profilVitesse.decc_tempsAcc = (floor(traj->profilVitesse.decc_tempsAcc / TE) + 1) * TE;
            traj->profilVitesse.decc_vmax = (3.0 / (2.0 - traj->profilVitesse.decc_alpha)) * (distance_restante / traj->profilVitesse.decc_tempsAcc);
    ...
    {
        /* Decceleration */
        flag_fin_etat = ASSER_TRAJ_Decceleration(&traj->profilVitesse, vitesse_initiale, flag_TD, TE, PAS, traj->profilVitesse.decc_alpha, traj->profilVitesse.decc_vmax, traj->profilVitesse.decc_tempsAcc, &vitesse_possible);
        //vitesse_possible = vitesse_initiale;
        if (flag_fin_etat == 1)
        {
    ...
    vitLimiteCourbure = traj->profilVitesse.vmax / ( 1.0 + fabs(traj->profilVitesse.diffThetaCourant) * facteurVitesseAngulaireMax);
    ...
    ASSER_TRAJ_LogAsser("distance_restante", NBR_ASSER_LOG_VALUE, chemin.profilVitesse.distNormaliseeRestante * chemin.distance);
