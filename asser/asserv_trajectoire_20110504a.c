...
static void                     ASSER_TRAJ_Acceleration_np1(float a, float A_n, float v_n, float d, float *A_np1, float *v_np1);
...
static void ASSER_TRAJ_Acceleration_np1(float a, float A_n, float v_n, float d, float *A_np1, float *v_np1)
{
    float A_np1, v_np1, t_n;
    float q_2, p_3, k;

    q_2 = ( ((3.0 * A_n * v_n) / (2.0 * a * a)) + ((3.0 * d) / a) - (pow( (A_n/a), 3.0) / 4.0) ) / 2.0;
    p_3 = ( ((3.0 * A_n * A_n)/(4 * a * a)) - ((3 * v_n) / a) ) / 3.0;
    k = pow( (q_2 + sqrt( (q_2 * q_2) - (p_3 * p_3 * p_3) )), 1.0/3.0) + pow( (q_2 - sqrt( (q_2 * q_2) - (p_3 * p_3 * p_3) )), 1.0/3.0);
    t_n = k - (A_n / (2.0 * a));
...
static void ASSER_TRAJ_GabaritVitesse(Trajectoire *traj, unsigned int flag_init)
{
    ...
    while (flag_finTraj == 0)
    {
        ...
        //ASSER_TRAJ_Acceleration_np1(float a, float A_n, float v_n, float d, float *A_np1, float *v_np1);
        //g_tab_gabarit_acceleration[]
